...
export const Signup = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (Object.values(errorForm).every((val) => val === false)) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (disableSubmit != true) {
      if (location.pathname === "/signup/root") {
        registerRoot(formData,setFormData)
      }
    }
  };
  ...
  return (
      <div className="w-1/3 border border-primaryColor rounded-lg bg-white shadow-2xl">
        <form
